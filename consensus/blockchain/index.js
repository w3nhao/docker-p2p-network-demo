...
class Blockchain {
  ...
  commitBlock(height, peers, commitIP) {
    if (
      ...
      console.log(JSON.stringify(this.commitCache));
    } else {
      for (let i = 0; i < this.commitCache.length; i++) {
        if (this.commitCache[i].height === height) {
          ...
          if (
            ...
            const restCommits = [];
            for (let j = i + 1; j < this.commitCache.length - 1; j++) {
              restCommits.push(this.commitCache[j]);
            }
